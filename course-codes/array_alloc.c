...
/* 
   Simple wrappers for allocating arrays so that the elements are
   ...
   where <dimensionality is of the form nd, where n is the number of
   dimensions, and <type> is the data type

   Each function returns an pointer with the appropriate degre of
   indirectino for the dimensionality required . On error NULL is
   returned.

   The arguments to the functions are the dimensions in natural order.

   Ian Bush, 2010 for SESG6025, University of Southampton, 
   very minor modifications by Hans Fangohr.
